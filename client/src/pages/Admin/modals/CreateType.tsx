...
import {createTypeApi} from "../../../api/apiDevice";
...
const CreateType: React.FC<PropsType> = ({onHide, show}) => {
    ...
    const onSubmit = (data: { addType: string }) => {
        dispatch(createTypeApi({name: data.addType}))
        reset({addType: ''})
        onHide()
    }
    return (
                        ...
                        <Controller
                            ...
                            render={({field: {onChange, value}}) => (
                                <TextField
                                    ...
                                    onChange={onChange} value={value} label={"Введит название типа"}/>
                            )}
                        />
